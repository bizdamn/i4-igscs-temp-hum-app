...
export default function DevicePage({ tempArray, humArray, deviceCalibration }) {
  ...
  const [current_temperature_calibration, setCurrent_temperature_calibration] = useState(deviceCalibration[0].temperature_calibration);
  ...
  async function temperatureFilter() {
    closeSnackbar()
    try {
      const { data } = await axios.post('https://i4-server-temp-hum-chart.igscsi4server.com/', {
...
export async function getServerSideProps(ctx) {
  const { id } = ctx.query;
  await db.connect();
  const result = await Entries.find({ deviceName: id }).lean()
  const deviceCalibration = await DeviceCalibration.find({ deviceName: id }).lean();
  await db.disconnect();
